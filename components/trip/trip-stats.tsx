...
function getTotalCost(stops: Stop[]): { eur: number; gbp: number; other: number } {
  return stops.reduce(
    (acc, stop) => {
...
export function TripStats({ stops, route, startDate, endDate }: TripStatsProps) {
      ...
      {(costs.eur > 0 || costs.gbp > 0) && (
        <Card className="col-span-2 sm:col-span-3 lg:col-span-6">
          <CardContent className="flex flex-wrap gap-4 p-3">
            <span className="text-xs text-muted-foreground">Accommodation Costs:</span>
            {costs.gbp > 0 && <span className="text-sm font-medium">£{costs.gbp}</span>}
            {costs.eur > 0 && <span className="text-sm font-medium">€{costs.eur}</span>}
          </CardContent>
        </Card>
      )}
    </div>
